...
# Load data
file_path = "solution_concept_nash_comparisons_results.json"
...
def process_multiple_progressions(progression_list, sigma_list, lambda_list, max_len=1000):
    ...
    for progression, Sigma_set, lambda_mu_set in zip(progression_list, sigma_list, lambda_list):
        ...
        for state in padded_progression:
            ...
            step_utilities.append(np.mean(agent_utils))  # average over agents
...
for a in num_agents_list:
    for s in num_stocks_list:
        ...
        progression_ours, progression_comparisons = [], []
        progression_nash, progression_zeroth = [], []
        Sigma_list_all = []
        Lambda_list_all = []
        ...
        for entry in data[a][s]:
            ...
            nash_final_points_zeroth.append(state_progression_nash_zeroth[-1])

            progression_ours.append(state_progression_ours)
            progression_comparisons.append(state_progression_ours_comparisons)
            progression_nash.append(state_progression_nash)
            progression_zeroth.append(state_progression_nash_zeroth)
